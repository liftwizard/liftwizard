...
public class VerifyAssertSizeToAssertJ extends Recipe {

    private static final MethodMatcher VERIFY_ASSERT_SIZE_MATCHER = new MethodMatcher(
        "org.eclipse.collections.impl.test.Verify assertSize(java.lang.String, int, java.lang.Iterable)"
    );

    @Override
    public String getDisplayName() {
        return "Verify.assertSize() → AssertJ assertThat().hasSize()";
    }

    @Override
    public String getDescription() {
        return "Transforms Eclipse Collections Verify.assertSize(message, size, collection) calls to AssertJ assertThat(collection).as(message).hasSize(size) for better test readability and modern assertion style.";
    }
                ...
                public J visitMethodInvocation(J.MethodInvocation method, ExecutionContext ctx) {
                    ...
                    if (VERIFY_ASSERT_SIZE_MATCHER.matches(m)) {
                        JavaTemplate template = JavaTemplate.builder(
                            "Assertions.assertThat(#{any(java.lang.Iterable)}).as(#{any(java.lang.String)}).hasSize(#{any(int)})"
                        )
                        ...
                        Expression sizeArg = m.getArguments().get(1);
