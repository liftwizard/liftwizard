...
{
    private static final MethodMatcher NONE_SATISFY_MATCHER = new MethodMatcher("org.eclipse.collections.api..* noneSatisfy(..)", true);
    private static final MethodMatcher ANY_SATISFY_MATCHER = new MethodMatcher("org.eclipse.collections.api..* anySatisfy(..)", true);
    ...
    public TreeVisitor<?, ExecutionContext> getVisitor()
    {
        ...
        return Preconditions.check(check, new JavaVisitor<ExecutionContext>()
        {
            ...
            public Expression visitExpression(Expression expression, ExecutionContext ctx)
            {
                ...
                if (unary.getOperator() != J.Unary.Type.Not || !(unary.getExpression() instanceof J.MethodInvocation))
                {
                    return e;
                }
                ...
                if (NONE_SATISFY_MATCHER.matches(methodInvocation))
                {
                    ...
                    return methodInvocation.withName(
                            methodInvocation.getName().withSimpleName("anySatisfy")
                ...
                else if (ANY_SATISFY_MATCHER.matches(methodInvocation))
                {
                    ...
                    return methodInvocation.withName(
                            methodInvocation.getName().withSimpleName("noneSatisfy")
                    ).withPrefix(unary.getPrefix());
                }
