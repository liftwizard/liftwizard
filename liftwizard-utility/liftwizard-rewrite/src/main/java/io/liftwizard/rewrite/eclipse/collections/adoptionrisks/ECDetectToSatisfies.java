...
/**
 * Note: This recipe cannot be converted to Refaster style because Refaster templates require
 * exact type matching. Eclipse Collections' detect/anySatisfy/noneSatisfy methods use
 * org.eclipse.collections.api.block.predicate.Predicate, but OpenRewrite's JavaTemplate
 * matching won't match java.util.function.Predicate even though EC's Predicate extends it.
 * The JavaVisitor approach with MethodMatcher is more flexible for this pattern.
 */
public class ECDetectToSatisfies extends Recipe {
    ...
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return Preconditions.check(new UsesMethod<>(DETECT_MATCHER), new ECDetectToSatisfiesVisitor());
    }

    private static final class ECDetectToSatisfiesVisitor extends JavaVisitor<ExecutionContext> {
        ...
        public J visitBinary(J.Binary binary, ExecutionContext ctx) {
            ...
            return detectCall.withName(detectCall.getName().withSimpleName(newMethodName)).withPrefix(b.getPrefix());
        }
