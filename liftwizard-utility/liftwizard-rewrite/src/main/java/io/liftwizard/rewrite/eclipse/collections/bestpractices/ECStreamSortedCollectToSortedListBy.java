...
/**
 * Transforms stream sorted with Comparator.comparing and collect to toSortedListBy.
 *
 * <p>Example:
 * <pre>{@code
 * // Before
 ...
 *
 * // After
 * list.toSortedListBy(Person::getAge);
 * }</pre>
 *
...
public class ECStreamSortedCollectToSortedListBy extends Recipe {
	...
	private static final MethodMatcher SORTED_MATCHER = new MethodMatcher(
	...
	public String getDescription() {
		return (
			...
			+ "the toSortedListBy method directly on RichIterable."
		);
	}
	...
	private static final class StreamSortedCollectToSortedListByVisitor extends JavaIsoVisitor<ExecutionContext> {
		...
		public J.MethodInvocation visitMethodInvocation(J.MethodInvocation method, ExecutionContext ctx) {
			...
			J.Identifier toSortedListByName = methodInvocation.getName().withSimpleName("toSortedListBy");

			return methodInvocation
				.withSelect(collectionExpr.withPrefix(collectSelect.getPrefix()))
				.withName(toSortedListByName)
