...
final class GarbageFreeLambdaVisitor extends JavaIsoVisitor<ExecutionContext> {
	...
	static final List<Path> ECLIPSE_COLLECTIONS_API_CLASSPATH = locateJars("org.eclipse.collections.api.RichIterable");

	private static List<Path> locateJars(String... classNames) {
		List<Path> paths = new ArrayList<>(classNames.length);
		for (String className : classNames) {
			try {
				URL location = Class.forName(className).getProtectionDomain().getCodeSource().getLocation();
				paths.add(Paths.get(location.toURI()));
			} catch (ReflectiveOperationException | URISyntaxException ignored) {
				// Best effort: if the class isn't loadable here, the template parser will fall back to no classpath.
			}
		}
	...
	public J.MethodInvocation visitMethodInvocation(J.MethodInvocation method, ExecutionContext ctx) {
		...
		JavaTemplate template = JavaTemplate.builder(templateSource)
			...
			.contextSensitive()
			.javaParser(JavaParser.fromJavaVersion().classpath(ECLIPSE_COLLECTIONS_API_CLASSPATH))
			.build();

		if (!result.typeFqn().startsWith("java.lang.")) {
			this.maybeAddImport(result.typeFqn());
		}
