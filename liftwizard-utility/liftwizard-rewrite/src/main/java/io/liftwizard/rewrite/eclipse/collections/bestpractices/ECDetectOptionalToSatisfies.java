...
public class ECDetectOptionalToSatisfies extends Recipe {
    ...
    public String getDescription() {
        return "Converts `iterable.detectOptional(predicate).isPresent()` to `iterable.anySatisfy(predicate)` and `!iterable.detectOptional(predicate).isPresent()` to `iterable.noneSatisfy(predicate)` for Eclipse Collections types.";
    }
        ...
        return Preconditions.check(
            ...
            new JavaVisitor<ExecutionContext>() {
                ...
                public Expression visitExpression(Expression expression, ExecutionContext ctx) {
                    ...
                    // Handle negated case: !detectOptional().isPresent() -> noneSatisfy()
                    if (e instanceof J.Unary unary) {
                        if (unary.getOperator() == J.Unary.Type.Not) {
                            J.MethodInvocation detectOptionalCall = this.getDetectOptionalCall(unary.getExpression());
                            if (detectOptionalCall != null) {
                                return detectOptionalCall
                                    .withName(detectOptionalCall.getName().withSimpleName("noneSatisfy"))
                                    .withPrefix(unary.getPrefix());
                            }
                        }
                    }

                    // Handle positive case: detectOptional().isPresent() -> anySatisfy()
                    J.MethodInvocation detectOptionalCall = this.getDetectOptionalCall(e);
                    if (detectOptionalCall != null) {
                        return detectOptionalCall
                            .withName(detectOptionalCall.getName().withSimpleName("anySatisfy"))
                ...
                private J.MethodInvocation getDetectOptionalCall(Expression expression) {
                    if (!(expression instanceof J.MethodInvocation methodInvocation)) {
                        return null;
                    }

                    // Check if this is a call to isPresent() on any Optional type
                    if (
                    ...
                    // Check if the select is a detectOptional() call
