 ...
 * that creates metrics. Metrics created after a later bundle's {@code initialize} but before this
 * bundle's {@code run} use a {@link NoOpStatsDClient} placeholder until {@code run} swaps in the
 * real client.
 */
public class DogStatsDPushBundle implements ConfiguredBundle<DogStatsDPushFactoryProvider> {
	...
	public void initialize(@Nonnull Bootstrap<?> bootstrap) {
		...
		existing.getMetrics().forEach((name, metric) -> this.registry.register(name, metric));
